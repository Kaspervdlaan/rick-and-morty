...
const FILTER_META = {
  ...
  species: { label: "Species", type: "select", options: speciesOptions },
  type: { label: "Type", type: "select", options: typeOptions },
  gender: { label: "Gender", type: "select", options: genderOptions },
};
...
export default function CharacterFilterBar({
  ...
  const [activeKeys, setActiveKeys] = useState([]); // e.g. ["status","species"]
  ...
  const availableKeys = useMemo(() => {
    return Object.keys(FILTER_META).filter((k) => !activeKeys.includes(k));
  }, [activeKeys]);
...
function FilterPill({ k, value, onChange, onRemove }) {
